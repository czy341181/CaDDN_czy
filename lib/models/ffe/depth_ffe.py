...
class DepthFFE(nn.Module):
    ...
    def __init__(self, model_cfg):
        ...
        # Create modules
        ddn_cfg = self.model_cfg['DDN']

        self.ddn = ddn.__all__[ddn_cfg['name']](
            backbone_name=ddn_cfg['backbone'],
            feat_extract_layer=ddn_cfg['feat_extract_layer'],
            num_classes=self.disc_cfg["num_bins"] + 1,
            pretrained_path=ddn_cfg['pretrained_path']
        )
        self.channel_reduce = BasicBlock2D(**self.model_cfg['CHANNEL_REDUCE'])
        self.ddn_loss = DDNLoss(disc_cfg=self.disc_cfg,
                                downsample_factor=self.downsample_factor,
                                **self.model_cfg['DDN_LOSS'])
    ...
    def forward(self, batch_dict, istrain=True):
        ...
        """
        # Pixel-wise depth classificationpython
        images = batch_dict["images"]
        ddn_result = self.ddn(images)
        image_features = ddn_result["features"] #[2, 256, 94, 311]
        depth_logits = ddn_result["logits"] #[2, 81, 94, 311]

        # Channel reduce
        if self.channel_reduce is not None:
            image_features = self.channel_reduce(image_features) #[2, 256, 94, 311]-->[2, 64, 94, 311]

        # Create image feature plane-sweep volume
        frustum_features = self.create_frustum_features(image_features=image_features,
                                                        depth_logits=depth_logits)  #[2, 64, 80, 94, 311]
